...
and processed with the run_analysis.R script, described in the README.md file, to join data and to aggregate mean values, as described below.

To run the script a directory named UCI HAR Dataset must be present in the working directory. The directory must preserve the original structure of the dataset, including subdirectories test and train.
...
(*) The above description was reproduced from The Machine Learning Repository at UCI. Cite:
